    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        print('optobjectname:', opt.object_name)
                        if names[int(cls)] == opt.object_name:
                            count = count + 1
                        if count >= 50:
                            while not rospy.is_shutdown():
                                object_state_pub.publish(True)
                                print("find_object!!!")
            ...
            if view_img:
                ...
                k = cv2.waitKey(1)  # waitkey代表读取键盘的输入，括号里的数字代表等待多长时间，单位ms。 0代表一直等待
                if k == 27:  # 键盘上Esc键的键值
...
if __name__ == '__main__':
    ...
    parser.add_argument('--object-name', type=str, help='object-name')
